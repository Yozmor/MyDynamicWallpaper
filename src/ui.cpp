...
void UpdateAstroInfo(HWND hwndDlg) {
    ...
    if (se.polarDay || se.polarNight) {
        // Если текущий день полярный, времена не выводим
    } else {
    ...
    // Получаем диапазоны полярных периодов
    ...
    // Формируем одну строку: "Полярный день: с ... по ...; Полярная ночь: с ... по ..."
...
void UpdateUIForTheme(HWND hwndDlg) {
    ...
    const auto& sectors = g_theme.getSeasonSectors(g_currentSeason);
    ...
    if (!sectors.empty()) {
        ShowPreview(GetDlgItem(hwndDlg, IDC_PREVIEW), sectors[0].image);
    }
}
        ...
        case IDC_SEASON_COMBO:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                if (sel != CB_ERR) {
                    ...
                    SendMessageW(hSectorCombo, CB_SETCURSEL, 0, 0);
                    g_currentSector = 0;
                    if (!sectors.empty()) ShowPreview(GetDlgItem(hwnd, IDC_PREVIEW), sectors[0].image);
                }
            }
            break;
        case IDC_SECTOR_COMBO:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                if (sel != CB_ERR) {
                    g_currentSector = sel;
                    const auto& sectors = g_theme.getSeasonSectors(g_currentSeason);
                    if (sel < (int)sectors.size()) ShowPreview(GetDlgItem(hwnd, IDC_PREVIEW), sectors[sel].image);
                }
            }
            break;
        case IDC_PREV_BUTTON: {
            HWND hSectorCombo = GetDlgItem(hwnd, IDC_SECTOR_COMBO);
            int count = SendMessageW(hSectorCombo, CB_GETCOUNT, 0, 0);
            if (count > 0) {
