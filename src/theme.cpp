...
// Вспомогательная функция конвертации UTF-8 -> wide
...
std::wstring Theme::computeImage(const SYSTEMTIME& now) const {
    ...
    std::vector<ActiveSector> active = buildActiveSectors(now, season);

    double currentMinutes = now.wHour * 60 + now.wMinute;
    for (const auto& as : active) {
        if (as.startMinute <= currentMinutes && currentMinutes < as.endMinute) {
            return as.image;
        }
    }
    if (!active.empty()) return active.front().image;
    return L"";
}
