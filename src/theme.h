...
public:
    ...
    // Новая структура для временных интервалов секторов
    struct ActiveSector {
        double startMinute;
        double endMinute;
        std::wstring image;
    ...
    // Возвращает все активные сектора для указанной даты (с интервалами)
    std::vector<ActiveSector> getActiveSectorsForDate(const SYSTEMTIME& now) const;
